...
# vivado_synth.sh — drive Vivado synth_design + place + route on one or
# more HLS-synthesized kernels. Without the U50 platform, this gives
# the user-logic place-and-route numbers (excluding the platform shell)
# which is what reviewers care about for area and timing.
set -uo pipefail
...
KERNEL="${1:-retrans}"
HLS_PROJ="${OPENURMA_ROOT}/build/hls/${KERNEL}_proj"
RTL_DIR="${HLS_PROJ}/solution1/syn/verilog"
OUT="${OPENURMA_ROOT}/build/vivado/${KERNEL}"
mkdir -p "${OUT}"
...
echo "[vivado] synth+impl ${KERNEL} → ${OUT}"
(cd "${OUT}" && vivado -mode batch -source "${OUT}/${KERNEL}_synth.tcl" -nojournal -log "${OUT}/vivado.log" 2>&1) | tail -10
echo "Reports:"
echo "  ${OUT}/utilization_route.rpt"
echo "  ${OUT}/timing_summary_route.rpt"
