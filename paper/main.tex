...
selective-retransmit path uses the spec-defined TPSACK
...
either a TAACK that explicitly NAKs PSNs (selective) or the
RTO element firing its single-port timer (Go-Back-N). Each
...
\item \textbf{Jetty Group (\S8.2.2.1 Type 3)} ---
  the target-side dispatcher dispatches incoming Sends to one of $N$
...
element implementing the transport-bypass path and a topology variant
the load/store topology that wires
the load/store transmit pipeline (doorbell, engine, header build, encapsulation)
--- 5 stages cold. Linking the new variant into the same
the same SystemC harness and posting a single
a load request, the first wire flit emerges at
...
\begin{itemize}\itemsep -1pt
  \item pointer-chase --- linked-list traversal, dep-chain
  ...
  \item distributed-barrier --- $N$-process FAA on a shared
    counter; latency-bound synchronisation primitive.
  \item hash-probe --- random-key memcached-style lookup.
  \item CAS-lock --- single-contended-line CAS lock
    acquisition.
  \item graph-BFS --- mixed READ/WRITE traversal of remote
    vertices (~75\% READ, 25\% WRITE).
  \item send-receive pingpong --- two-sided pingpong via SEND/RECV
    verbs, exercising the JFR + RQE path.
  \item bulk-read --- one-sided large-payload READ
    (8~B $\rightarrow$ 64~KB); used for payload-size scaling.
  \item bulk-write --- the WRITE-only dual.
\end{itemize}
...
an in-order ARM CPU model (no pipeline contribution by
...
bulk-read workload at UNO vs ROL. At small payloads ROL
...
a per-packet loss-rate parameter. On drop, GBN (RoCE) retransmits the entire
...
p99 tail latency across a six-point loss range from
$10^{-4}$ to $10^{-1}$. At 5\%
...
\begin{figure*}[t]
  ...
  C++ simulator's the congestion-controller module. cwnd trajectory
...
core multiplexer instances (the dispatch and
transmit muxes, both 4-input round-robin mergers) account for
8.8~KLUT each --- 14.4\% of OpenURMA's total. The same source
the multiplexer synthesises to 3.6~KLUT in OpenRoCE; the 2.4$\times$
...
the on-NIC memory read and write elements --- the in-context
...
intended FPGA wiring uses FPGA-targeted memory-access variants
that issue AXI-MM transactions to the platform shell; the
software-emulator byte-array layer is a development convenience.
...
path implements both GBN and a selective TPSACK-driven
retransmit slot (\S\ref{subsec:impl-retrans}); we treat the above
...
ordering surface (UNO + NO); we implement TPG-style spray as one of the multi-path
...
(\S\ref{subsec:design-ordering}) is the specification-level
