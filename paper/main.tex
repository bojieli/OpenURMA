...
SystemC NIC into gem5 as a memory-mapped device. Three load-bearing
changes were needed. The SystemC libraries must be rebuilt against
gem5's embedded SystemC headers, which are ABI-incompatible with
the upstream Accellera build the cycle-accurate back-end normally
uses. The NIC's address aperture must be carved out of system
memory because gem5's memory-bus crossbar cannot route overlapping
ranges, and the controller's initialisation hook must explicitly
call the crossbar's range-change notifier so the bus latches the
new range before the binary loader's first write. A dual-node
configuration brings up two ARM CPUs with an EtherLink between
their controllers, runs SE-mode binaries that exercise the
aperture through ordinary load and store instructions, and
pre-maps the aperture into each process's page table so loads
bypass gem5's virtual-memory subsystem. The current scaffold
validates the end-to-end CPU-to-remote-controller path; full-system
Linux integration is follow-on work.
...
(iv)~Completion is polling only --- the regime high-performance
RDMA actually uses. Event-driven completion adds MSI-X, ISR,
scheduler, and wakeup, which a single analytical parameter cannot
represent faithfully; FastWake~\cite{li2023:fastwake} characterises
that host-side path directly. The gem5 scaffold quantifies the gap:
on the same 32-op WRITE workload, direct memory-mapped completion
polling measures \textbf{21~ns mean / 40~ns max} (the NIC-side
floor); a kernel ioctl wrapper measures \textbf{437~ns mean /
438~ns max} (416~ns of kernel-driver tax); the event-driven
variant where the driver sleeps in post-send and wakes on
completion measures \textbf{967~ns mean / 994~ns max} (an
additional 530~ns of scheduler overhead). All three use an
in-order ARM CPU model and a loopback-ack injector standing in
for the not-yet-implemented target-side ack pipeline. The
21/437/967~ns chain is the first controlled end-to-end measurement
of the polled-vs-event gap on a UB-class transport, with each
value adding the OS path component above the layer below it.
...
scaffold. A multi-tenant workload forks $N{\in}\{1,4,16,64\}$
child processes, each opening the URMA character device and
posting 8 polled WRITE ops; per-op average latency stays within
14\% of the single-process baseline as $N$ grows
64$\times$ (5915 / 6294 / 6423 / 6727~ns). The per-NIC SystemC
state is unchanged --- only per-process Linux bookkeeping (file
table, mmap) scales linearly --- which is the empirical face of
the $O(N{+}M)$ claim.
