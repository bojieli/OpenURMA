...
int main(int argc,char**argv){
  ...
  char* buf=aligned_alloc(4096,1<<16); memset(buf,0,1<<16);
  sbuf=buf; rbuf=buf+MSG;   // two message slots in the registered region
  urma_seg_cfg_t sc; memset(&sc,0,sizeof sc); sc.va=(uint64_t)(uintptr_t)buf; sc.len=1<<16; sc.token_value.token=0xDEADBEEF;
  ...
  if(is_srv){
    ...
    // large multi-page value (8000 B = ~2 pages) RPC: PUT then GET + verify integrity
    { static char bigv[8000]; for(int i=0;i<8000;i++) bigv[i]=(char)('A'+(i%26));
      char m[MSG]; m[0]='P'; m[1]=6; m[2]=(8000>>8)&0xff; m[3]=8000&0xff;
      memcpy(m+4,"bigkey",6); memcpy(m+10,bigv,8000);
      post_recv(); send_msg(m,4+6+8000); int n=recv_msg(); total++;
      ...
      int gok=(n>0 && rbuf[0]=='O' && vn==8000 && memcmp(rbuf+4,bigv,8000)==0);
      total++; pass+=gok;
